...
export async function fetchArticleBySlug(slug) {
    const url = `${API_BASE}/api/articles/${encodeURIComponent(slug)}`;
    const resp = await fetch(url);
    if (!resp.ok) {
...
export async function fetchSearch(query, status) {
    ...
    const url = `${API_BASE}/api/articles/search?${params.toString()}`;
    const resp = await fetch(url);
    if (!resp.ok)
...
export async function updateArticle(id, articleData, token) {
    const url = `${API_BASE}/api/articles/${encodeURIComponent(id)}`;
    const resp = await fetch(url, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
